...
# ------------------------
# 2. Plot side-by-side comparisons
# ------------------------
...
# 2.1 Child Count KS Comparison (boxplots)
...
# 2.2 JS Divergence (Statistical Fidelity)
...
# 2.3 Distance Metrics (normalized) - bar charts side-by-side
def plot_dual_barplot(df1, df2, metric_col, value_col, title, ylabel, filename):
    # Normalize within each dataset
    df1 = df1.copy()
    df1['normalized'] = df1[value_col] / df1.groupby(metric_col)[value_col].transform('max')
    df2 = df2.copy()
    df2['normalized'] = df2[value_col] / df2.groupby(metric_col)[value_col].transform('max')
    ...
    axes[0].set_title('Northwind: '+title)
    axes[0].set_ylabel(ylabel)
    axes[0].set_xlabel(metric_col.capitalize())
    axes[0].grid(True)
    axes[0].legend(title='Generator')

    sns.barplot(x=metric_col, y='normalized', hue='generator', data=avg2, ax=axes[1], errorbar=None)
    axes[1].set_title('Basketball: '+title)
    axes[1].set_xlabel(metric_col.capitalize())
    axes[1].grid(True)
...
# 2.4 Constraint Adherence - bar charts side-by-side
def plot_dual_simple_bar(df1, df2, title, ylabel, filename):
    fig, axes = plt.subplots(1, 2, figsize=(10, 5), sharey=True)
    sns.barplot(x='generator', y='value', data=df1, ax=axes[0], errorbar=None)
    axes[0].set_title('Northwind: '+title)
    axes[0].set_ylabel(ylabel)
    axes[0].set_xlabel('Generator')
    axes[0].grid(True)

    sns.barplot(x='generator', y='value', data=df2, ax=axes[1], errorbar=None)
    axes[1].set_title('Basketball: '+title)
    axes[1].set_xlabel('Generator')
    axes[1].grid(True)
...
print("\n✅ Dual Dataset Comparison Charts Generated at 'analysis_results_both/' ✅")
